...
interface IncidentDispensedItemsModalProps {
  open: boolean;
...
export default function IncidentDispensedItemsModal({
  open,
  onOpenChange,
  incident,
}: IncidentDispensedItemsModalProps) {
  ...
  const locationId = incident?.locationId ?? undefined;

  const { data: existingTransactions = [], isLoading: loadingExisting } = useQuery({
    queryKey: ["/api/inventory-transactions", { documentType: "incident", documentId: incidentId }],
    queryFn: async () => {
      const params = new URLSearchParams({ documentType: "incident", documentId: incidentId });
  ...
  const patientName = incident
    ? isOperationsRole
      ? incidentPersonCardSubtitle(incident, true)
      : incident.patient?.employee
        ? `${incident.patient.employee.firstName || ""} ${incident.patient.employee.lastName || ""}`.trim() ||
          "Employee"
        : "Employee"
    : "";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Record supplies or medications given for this incident. Incident: {patientName}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          {loadingExisting ? (
            ...
            <div>
              <h4 className="text-sm font-medium mb-2">Already recorded for this incident</h4>
