...
import { FleetFormDialog, DeleteFleetDialog } from "./FleetDialogs";

export function FleetSection({ isAdmin }: { isAdmin: boolean }) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [createOpen, setCreateOpen] = useState(false);
  const [editRow, setEditRow] = useState<FleetUnitRow | null>(null);
  const [deleteRow, setDeleteRow] = useState<FleetUnitRow | null>(null);
  const [includeInactive, setIncludeInactive] = useState(false);

  const { data: units = [], isLoading } = useQuery<FleetUnitRow[]>({
    queryKey: ["/api/fleet", { includeInactive }],
    queryFn: async () => {
      const q = includeInactive ? "?includeInactive=true" : "";
      const res = await fetch(`/api/fleet${q}`, { credentials: "include" });
  ...
  const stockSummaryQueries = useQueries({
    queries: units.map((a) => ({
      queryKey: ["/api/fleet", a.id, "stock-summary"] as const,
      queryFn: async () => {
        const res = await fetch(`/api/fleet/${a.id}/stock-summary`, { credentials: "include" });
        if (!res.ok) throw new Error(await res.text());
      ...
      staleTime: 60_000,
      enabled: !isLoading && units.length > 0,
    })),
  });
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Operational status and home store / site</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-uventorybiz-gray py-8 text-center">
              ...
              {isAdmin && " Add one to start tracking vehicle inventory."}
            </p>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Vehicle stock</TableHead>
                ...
                <TableBody>
                  {units.map((row, idx) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell>
                        <Link href={`/assets/fleet/units/${row.id}`}>
                      ...
                      <TableCell className="text-right">
                        <div className="inline-flex flex-row">
                          <Link href={`/assets/fleet/units/${row.id}`} className="flex">
