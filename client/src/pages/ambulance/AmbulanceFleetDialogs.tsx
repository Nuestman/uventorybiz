...
function generateAmbulanceCodeFromName(name: string): string {
  const cleaned = name
    .trim()
    .toUpperCase()
    .replace(/[^A-Z0-9\s-]/g, " ")
  ...
  const meaningful = tokens.filter((t) => !["AMBULANCE", "UNIT", "THE"].includes(t));
  ...
  const explicitNumberToken =
    ...
    tokens.find((t) => /(AMB|UNIT)[-_]?\d+/.test(t));
...
export function AmbulanceFormDialog({
  ...
  const [ambulanceOpsStatus, setAmbulanceOpsStatus] = useState<string>("available");
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && initial) {
      ...
      setAmbulanceOpsStatus(initial.ambulanceOpsStatus ?? "available");
      ...
      setFleetNumber("");
      setAmbulanceOpsStatus("available");
      setStatus("active");
  ...
  useEffect(() => {
    ...
    setLocationCode(generateAmbulanceCodeFromName(locationName));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "create") {
        await apiRequest("POST", "/api/ambulances", body);
      } else if (initial) {
        await apiRequest("PATCH", `/api/ambulances/${initial.id}`, body);
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Inventory for this unit is tracked by location. Use Stock transfers to move supplies to or from posts and
            stores.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-2">
          <div className="grid gap-2">
            <Label htmlFor="amb-name">Display name</Label>
            <Input
              id="amb-name"
              value={locationName}
          ...
          <div className="grid gap-2">
            <Label htmlFor="amb-code">Short code</Label>
            <Input
              id="amb-code"
              value={locationCode}
              onChange={(e) => {
              ...
              placeholder="e.g. UG-AMB1"
            />
          </div>
          <div className="grid gap-2">
            <Label>Stationed at post / clinic</Label>
            <Select
              value={stationedAtLocationId || "__none__"}
              onValueChange={(v) => setStationedAtLocationId(v === "__none__" ? "" : v)}
            >
              <SelectTrigger>
                <SelectValue placeholder="Not assigned (multi-post / pooled)" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="grid gap-2">
            <Label htmlFor="amb-coverage">Coverage notes (optional)</Label>
            <Textarea
              id="amb-coverage"
              value={coverageNotes}
              onChange={(e) => setCoverageNotes(e.target.value)}
              placeholder="e.g. Surface unit covering all underground posts when not at main clinic"
              rows={3}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label htmlFor="amb-callsign">Call sign</Label>
              <Input id="amb-callsign" value={callSign} onChange={(e) => setCallSign(e.target.value)} />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="amb-plate">Registration</Label>
              <Input id="amb-plate" value={registrationPlate} onChange={(e) => setRegistrationPlate(e.target.value)} />
            </div>
          </div>
          <div className="grid gap-2">
            <Label htmlFor="amb-fleet">Fleet number</Label>
            <Input id="amb-fleet" value={fleetNumber} onChange={(e) => setFleetNumber(e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label>Operational status</Label>
              <Select value={ambulanceOpsStatus} onValueChange={setAmbulanceOpsStatus}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="grid gap-2">
            <Label htmlFor="amb-desc">Description</Label>
            <Textarea id="amb-desc" value={description} onChange={(e) => setDescription(e.target.value)} rows={2} />
          </div>
        </div>
        <DialogFooter>
...
export function DeleteAmbulanceDialog({
  ...
  const confirm = async () => {
    setPending(true);
    try {
      await apiRequest("DELETE", `/api/ambulances/${row.id}`);
      onDone();
    } catch (e: unknown) {
      toast({
        title: "Cannot delete",
    ...
    <AlertDialog open onOpenChange={(o) => !o && onClose()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This deletes the vehicle record only after all on-board inventory quantities are zero (transfer or adjust
